...
class Encodings {
	...
	static BoolExpr encodeCommonExecutions(Program p1, Program p2, Context ctx) {
		Filter filter = Filter.Or.of(Filter.Atom.memory, Filter.Atom.local);
		List<Event> p1Events = p1.getCache().getEvents(filter);
        List<Event> p2Events = p2.getCache().getEvents(filter);
	...
	static BoolExpr encodeReachedState(Program p, Model model, Context ctx) {
		...
		Set<RegWriter> executedEvents = p.getCache().getEvents(Event.class).stream()
