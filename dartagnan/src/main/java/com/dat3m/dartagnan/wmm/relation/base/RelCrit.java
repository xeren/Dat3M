    ...
    public TupleSet getMaxTupleSet(){
        if(maxTupleSet == null){
            ...
            for(Thread thread : program.getThreads()){
                for(Event lock : thread.getCache().getEvents(Filter.of(EType.RCU_LOCK))){
                    for(Event unlock : thread.getCache().getEvents(Filter.of(EType.RCU_UNLOCK))){
                ...
                for(Event unlock : thread.getCache().getEvents(Filter.of(EType.RCU_UNLOCK))){
                    if(lock.getCId() < unlock.getCId()){
                        ...
                        if(encodeTupleSet.contains(tuple)){
                            ...
                            for(Event otherLock : thread.getCache().getEvents(Filter.of(EType.RCU_LOCK))){
                            ...
                            for(Event otherUnlock : thread.getCache().getEvents(Filter.of(EType.RCU_UNLOCK))){
