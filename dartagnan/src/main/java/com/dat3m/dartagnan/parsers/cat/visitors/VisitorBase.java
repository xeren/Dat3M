...
public class VisitorBase extends CatBaseVisitor<Object> implements CatVisitor<Object> {
	...
	final VisitorRelation relationVisitor;
	final VisitorFilter filterVisitor;
	final Wmm wmm;
	final HashMap<String, Filter> filter = new HashMap<>();
	...
	public Object visitLetDefinition(CatParser.LetDefinitionContext ctx) {
		...
		if(r != null) {
			...
			filter.put(ctx.n.getText(), ctx.e.accept(filterVisitor));
		}
		return null;
	}
